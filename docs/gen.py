def gen_svg(out, window):
    ...
    with open(out, "w") as f:
        ...
        def add_polygon(ya1, ya2, yb1, yb2):
            print(f'<polygon points="{a_x},{ya1} {a_x},{ya2} {b_x},{yb2} {b_x},{yb1}" fill="gold" fill-opacity="0.0" stroke="transparent" />', file=f)
    ...
    从 T0 时刻开始：A 发送三个帧
    T3 时刻：B 完成接收第一个帧
    T6 时刻：A 完成接收第一个确认帧，开始发送第四个帧
    T7 时刻：A 完成发送第四个帧，完成接收第二个确认帧，开始发送第五个帧

    在 T0 到 T6 时间内，传输了 W 个帧的数据，耗费了 D+1/B+A/B+D 的时间
    之后都按照这个周期循环

    链路利用率等于 (W/B)/(2D+1/B+A/B)

    如果确认帧很小可以忽略，那么 A=0，链路利用率等于 W/(2BD+1)

    如果确认捎带在数据帧中传输，那么 A=1，链路利用率等于 W/(2BD+2)

    如果在 T0 到 T6 时间内来不及发送窗口大小的 W 个帧，那么 A 可以一直发送
    不会因为发送窗口满而停止发送，此时链路利用率等于 100%
